...
import model_uq_no_interventions
...
parser.add_argument("-i", "--pid", type=int, help="Row index of parameter samples") # added, default 0 as in the yaml file
...
# Checking pid
print('Row index corresponding to the parameter samples is {}'.format(args.pid))
...
# Setup#2: remove and think of loading
...
outdir = get_time_stamped_outdir(
                [params['output_location']['parent_dir'],
                    params['output_location']['results_dir']], params['results_file_postfix']+str(params["pid"])
        )
outfile = os.path.join(
        outdir, 
        params["generated_params_file_name"]
    )
...
for n in range(params['num_runs']):
    abm = model_uq_no_interventions.TorchABMCovid(params, device)
